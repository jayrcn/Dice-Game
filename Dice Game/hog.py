...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if score == 1:
            score_of_one = True
            #checks for the score of 1 in a single roll
        else:
            sum = score + sum
...
def picky_piggy(score):
    ...
    if score == 0:
     ...
     score % 6
    one_seventh_position =  (score - 1) % 6
    return (one_seventh // 10 ** one_seventh_position % 10) 
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        who_score = retrive_score(who)
        opponent_score = retrive_score(next_player(who))
        strategy = get_strategy(who)(who_score, opponent_score)
        new_score(who, take_turn(strategy, opponent_score, dice))
        who = next_player(who)
...
def announce_highest(who, last_score=0, running_high = 0 ):
    ...
    def say(score0, score1):
        ...
        if range > running_h and who==0:
            running_h = range
            print(range, "point(s)! That's a record gain for Player 0!")
        if range > running_h and who==1:
            running_h = range
            print(range, "point(s)! That's a record gain for Player 1!")
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    i, max_score = 1, 0
    while i <= 10:
        rolled_dice_avrg = make_averaged(roll_dice, trials_count)
        max_score_of_i = rolled_dice_avrg(i, dice)
        if max_score_of_i > max_score:
            max_score = max_score_of_i
            max_score_total = i
        i += 1
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if hog_pile(score + picky_piggy(opponent_score), opponent_score):
        return 0 
